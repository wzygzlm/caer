...
The following software is currently required to successfully compile cAER:
\begin{itemize}
\item A C/C++ compiler supporting the C11 and C++14 language standards. GCC 5.2 and newer, as well as LLVM/Clang 3.6 and newer satisfy that requirement.
\item cmake >= 2.6 for easy build management and cross-platform compilation support.
\item boost >= 1.50 (with system, filesystem and program\_options modules) for cross-platform file-access and networking support, as well as advanced C++ algorithms.
\item Mini-XML >= 2.7 for XML file parsing, used for the XML configuration file import and export features.
\item libcaer >= 2.4.0 for event definitions and USB/serial device access.
\end{itemize}
